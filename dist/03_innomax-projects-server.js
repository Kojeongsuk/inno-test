...
function innomaxProjectsRouter(pool) {
    ...
    router.post("/", async (req, res) => {
        ...
        try {
            ...
            await pool.query(`INSERT INTO innomax_projects (code_no, detail_json)
             VALUES ($1, $2)
             ON CONFLICT (code_no) DO UPDATE SET detail_json = EXCLUDED.detail_json`, [code_no, saveJson]);
    ...
    //수주건 일단 전체 조회함
    router.get("/innomax/projects", async (req, res) => {
        try {
    ...
    //특정코드 수주건만  조회함
    router.get("/innomax/project/:code_no", async (req, res) => {
        const { code_no } = req.params;
        try {
            const result = await pool.query(`SELECT detail_json FROM innomax_projects WHERE code_no = $1`, [code_no]);
